...
function MangaementJobs() {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, { _id, status }) => (
                status === "active" ?
                    (
                        <Tag onClick={() => changeSingleStatusId(_id, "inactive", fetchApiJobsManage, setRecordMain, api, statuss, keywords, sortKey, sortValue)} style={{ cursor: "pointer" }} icon={<CheckCircleOutlined />} color="success">
                            Hoạt Động
                        </Tag>
                    ) :
                    (
                        <Tag onClick={() => changeSingleStatusId(_id, "active", fetchApiJobsManage, setRecordMain, api, statuss, keywords, sortKey, sortValue)} style={{ cursor: "pointer" }} icon={<CloseCircleOutlined />} color="error">
                            Dừng Hoạt Động
                        </Tag>
                    )
            )
            ...
            render: (_, record) => (
                <Space size="small">

                    {
                        ...
                        userAdmin?.status && userAdmin.infoUser.permissions.includes('jobs-edit') === true && (
                            <>
                                <FormEdit record={record} fetchApiLoad={() => fetchApiJobsManage(setRecordMain, statuss, keywords, sortKey, sortValue)} />
                            </>
                        )
                    }
                    {
                        //Kiểm tra xem người dùng có quyền xóa hay không
                        userAdmin?.status && userAdmin.infoUser.permissions.includes('jobs-delete') === true && (
                            <>
                                <Popconfirm
                                    title="Xóa Công Việc"
                                    ...
                                    okText="Ok"
                                    cancelText="No"
                                    onConfirm={() => deleteSingleId(record._id, fetchApiJobsManage, setRecordMain, api, statuss, keywords, sortKey, sortValue)}
                                >
    ...
    const handleChangeStatus = async (value) => {
        ...
        fetchApiJobsManage(setRecordMain, valueStatus, keywords, sortKey, sortValue);
    ...
    const handleSort = async (value) => {
        ...
        fetchApiJobsManage(setRecordMain, statuss, keywords, sortKeySelect, sortValueSelect, "false");
        ...
        <Card>
            ...
            <div className="managementCategories mt-3 box-card">
                <div className="managementCategories__head mb-2 card-header">
                    <h2 className="title-header"><i>Công Việc</i></h2>
                </div>
                <div className="managementCategories__body card-body mt-2">
                    ...
                    <Table
                        ...
                        columns={columns} dataSource={recordMain}
                    />
                    {/* Hết Khối Table */}
                </div>
